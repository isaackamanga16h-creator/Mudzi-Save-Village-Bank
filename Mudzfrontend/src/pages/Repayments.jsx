...
import { getGroupMembers, recordRepayment } from '../api/service.jsx';
...
export default function Repayment() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await recordRepayment({ member_id: form.member_id, amount: parseFloat(form.amount) });
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
          <h2 className="text-xl font-bold mb-4 text-blue-400">Record Loan Repayment</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs text-slate-400 uppercase">Repayment Amount (MWK)</label>
              <input
                type="number"
                required
                className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white mt-1"
            ...
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 font-bold py-3 rounded-lg mt-4">
              Submit Repayment
            </button>
          </form>
        </div>
